...
public class RentalValidator implements Validator<Rental> {
    ...
    public void validate(Rental entity) throws ValidatorException {
        ...
        Optional.of(entity)
                .filter(rental -> this.dateIsValid(rental.getStartDate()))
                .orElseThrow(() -> new ValidatorException(String.format("Rental with id %d has an invalid start date: %s.", entity.getId(), entity.getStartDate())));
    }
